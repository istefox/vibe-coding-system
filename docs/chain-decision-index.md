...
- **ADR-0167** — `autopilot-disarm.sh` preserves `scope`/`published` instead of clearing them; the launch side (`conductor-scope-gate`) judges spent-vs-live so a relaunch after a pause reuses its bound instead of running unbounded → `docs/architecture/ADR-0167-400-autopilot-disarm-scope-unbounded.md`
