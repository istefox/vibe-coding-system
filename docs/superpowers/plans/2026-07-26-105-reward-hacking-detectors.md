...
- [ ] **Task 1 — RED harness.** `staging/plugin/scripts/tests/reward-hack-detectors.test.sh`.
...
- [ ] **Task 2 — the four detectors in `weakening-scan.sh`.** Extend the existing awk pipeline in
...
- [ ] **Task 3 — exclusions.** Implement each detector's declared exclusion per ADR-0051 §D5.
  → HB green.

- [ ] **Task 4 — measure the false-positive rate against this repo's real history.** Not a fixture.
...
- [ ] **Task 5 — surfacing (§D3).** `suspect_findings` array of `{file, detector, line}` in the
