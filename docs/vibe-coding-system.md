...
- **A scheduled `/loop` fire does not execute manual-only skills** (sec. 8, sec. 8.1, sec. 16): from CC v2.1.196, a scheduled fire only runs skills Claude is allowed to invoke on its own. Skills marked `disable-model-invocation: true` — along with built-in commands such as `/permissions` or `/clear`, skills withheld by a `skillOverrides` setting or a `Skill` deny rule, and MCP prompts — **reach Claude as plain text instead of executing**. Three staged skills carry that flag (`fastapi-react-vibe`, `goal-loop`, `research-prompt`), so `/loop 20m /research-prompt …` would silently do nothing but paste a string. This is a live trap, not a theoretical one. Source: `/en/scheduled-tasks`. **Correction (2026-07-25, issue #56):** this sentence used to read "Three staged skills … (`interview-driver`, `project-bootstrap`, `fastapi-react-vibe`)" and was wrong about all three — `project-bootstrap` was retired from staging by ADR-0025, and the other two had lost the flag in the issue-#29 refresh, which is what issue #56 restored. `goal-loop` and `research-prompt` gained it meanwhile without this line noticing. **Amended (2026-07-27, ADR-0067):** `interview-driver` no longer carries the flag and is out of the list — it is dispatched through the Skill tool by concept-to-code Step 1, which the flag blocks outright, so #56's restore made the chain's first step permanently unreachable. Removing it also closes this paragraph's own `/loop … /interview-driver` trap for that one skill. `skill-text-corrections.test.sh` section F pins the flag on `fastapi-react-vibe`, pins its **absence** on `interview-driver`, and checks this sentence's carrier list positively against the files rather than by banning a stale wording, so the claim and the files cannot drift apart again unobserved.
...
> `.claude/agent-memory/coder/topics/` shards before editing, bounds reconnaissance, caps full-suite
> runs at two with `tail -n 40` piping, makes the context7/LSP/eslint steps conditional on the tools
> being present, and ends with a hunk-by-hunk definition-of-done pass. Frontmatter unchanged. The
> block above stays as the historical record. Pinned by `coder-discipline.test.sh`.
...
> **Deployment note (2026-07-27, ADR-0067):** the template above still shows
> `disable-model-invocation: true`; the deployed and staged `interview-driver` deliberately does
> **not** carry it. That flag restricts a skill to user invocation, and `concept-to-code` Step 1 and
> Step H1 dispatch this skill through the Skill tool, so with the flag set the chain's first step
> fails outright (`cannot be used with Skill tool due to disable-model-invocation`). There is no
> settings-level exemption: frontmatter locks the skill state to on/name-only and `skillOverrides`
> can only disable further, never re-enable model invocation (verified against the CC 2.1.220
> binary). This supersedes issue #56 for this one file only — `fastapi-react-vibe`, `goal-loop` and
> `research-prompt` keep the flag, none of them being chain-invoked. The template is left as written
> for the historical record, per the ADR-0034 precedent. Full reasoning: ADR-0067.
