...
8. **Gate 0b (anonymize — conditional):** run `~/.claude/skills/clean-public-repo/scripts/detect-public-remote.sh <project-root>` (the script lives in the `clean-public-repo` skill, NOT in `concept-to-code/scripts/`).
   ...
   **Decision is exclusively the user's: never auto-applied.**
   ...
   (Gate CDX, the chain-start Codex-assist ask, was removed per ADR-0193/VCS-063 — the same
   choice is now asked at each reviewer dispatch site instead: RTF Step 0 item 6 for the three
   RTF sites, and a pre-dispatch ask in Step 5 for the checkpoint sites. `manifest.use_codex_review`
   is unchanged, still seeded `false` by `manifest-init.sh`.)
